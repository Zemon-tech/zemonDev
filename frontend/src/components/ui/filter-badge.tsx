...
const filterBadgeVariants = cva(
  "inline-flex items-center bg-gray-50 text-tremor-label text-muted-foreground",
  {
    variants: {
      variant: {
  ...
  if (variant === "avatar") {
    return (
      <span className={cn(filterBadgeVariants({ variant }), className)} {...props}>
        {avatar && (
          <img
            className="inline-block w-5 h-5 rounded-tremor-full"
            src={avatar}
            alt=""
          />
        )}
        ...
        {onRemove && (
          <button
            type="button"
            onClick={onRemove}
            className="flex w-5 h-5 items-center justify-center rounded-tremor-full text-muted-foreground hover:bg-muted hover:text-foreground"
            aria-label="Remove"
          >
    ...
    <span className={cn(filterBadgeVariants({ variant }), className)} {...props}>
      ...
      {onRemove && (
        <button
          ...
          onClick={onRemove}
          className={cn(
            "-ml-1.5 flex w-5 h-5 items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground",
