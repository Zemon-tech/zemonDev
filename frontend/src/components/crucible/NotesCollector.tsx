...
export default function NotesCollector() {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow border border-base-200 overflow-hidden">
      ...
      <div className="border-b border-base-200 p-2">
        ...
        <div className="flex gap-1">
          <textarea
            ...
            className="w-full p-2 border rounded text-sm min-h-[60px] max-h-[60px] focus:outline-none focus:ring-1 focus:ring-primary/50 resize-none"
          />
          <Button 
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {notes.length === 0 ? (
          ...
          <div className="space-y-2">
            {notes.map((note) => (
              <div key={note.id} className="p-2 bg-gray-50 rounded text-sm border border-gray-100 hover:border-gray-200 transition-colors">
                <div className="flex justify-between items-start gap-1">
                  <p className="flex-1 text-gray-800 text-sm">{note.content}</p>
                  <Button
                    variant="ghost"
                    size="icon"
                ...
                {note.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {note.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-1.5 py-0 bg-primary/10 text-primary text-[10px] rounded"
                      >
                        {tag}
                      </span>
                    ))}
